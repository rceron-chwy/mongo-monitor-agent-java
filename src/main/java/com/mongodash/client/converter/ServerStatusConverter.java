package com.mongodash.client.converter;
...
import com.mongodash.client.LockStats;
	...
	private static ServerStatus getStatusFromDBObject(DBObject object) throws ServerStatusException {
		...
		try {
			...
			if (!isMongos) {
				...
				serverStatus.setLocked(percent((Long) temp.get("totalTime"), (Long) temp.get("lockTime")));
			}

			serverStatus.setTime(sdf.format(new Date()));
		} catch (Exception e) {
			throw new ServerStatusException("Error while parsing server status: " + e.getMessage());
		}
